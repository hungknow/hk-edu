...
export async function createCourseEntity(courseData: { title: string; description: string; }): Promise<CourseEntity> {
  // Map description to name as per MongoDB entity definition
  const entityToInsert = {
    title: courseData.title,
    name: courseData.description, // Mapping description to name as per docs/mongodb/index.md
  };
  ...
  const tempSchema = zCourseEntity.pick({ title: true, name: true });
  ...
  return { _id: result.insertedId as any, ...validatedEntity };
}
